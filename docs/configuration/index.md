...
Configuration is loaded and merged by [`Config`](/architecture/config.md). The built-in `DEFAULT_CONFIG` provides defaults; user files layer on top, with platform-specific sections winning over generic ones.